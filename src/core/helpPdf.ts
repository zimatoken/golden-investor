...
/**
 * Генерация HTML-инструкции из HELP_CONTENT.
 *
 * Два варианта использования:
 *   1. openPrintableInstructions() — открыть окно с window.print() → PDF через браузер
 *   2. downloadInstructionsHTML() — скачать HTML-файл
 *
 * Кириллица работает без проблем — это нативный HTML.
 * Бандл не растёт — 0 KB.
 */

/**
 * Собирает полный HTML-документ инструкции.
 */
function buildHTML(): string {
  ...
  const titlePage = `
    <div class="page title-page">
      <h1>📘 Инструкция</h1>
      <p>Золотой Инвестор — тайминг-навигатор</p>
      <p>Курс молодого бойца</p>
      <div class="version">
        Версия 4.0 · 17.09.2026<br/>
        ${HELP_CONTENT.length} разделов
      </div>
    </div>
  `;

  const sectionsHTML = HELP_CONTENT.map((section) => renderSection(section)).join('');

  return `<!DOCTYPE html>
<html lang="ru">
<head>
  <meta charset="UTF-8" />
  <title>Инструкция — Золотой Инвестор</title>
  ${styles}
</head>
<body>
  ${titlePage}
...
/**
 * Открывает новое окно с HTML-инструкцией и автоматически
 * запускает диалог печати браузера.
 *
 * Пользователь: выбирает «Сохранить как PDF» в диалоге → получает PDF.
 */
export function openPrintableInstructions(): void {
  ...
  if (!win) {
    alert(
      'Не удалось открыть окно печати. Проверь настройки блокировки всплывающих окон.'
    );
    return;
  }
  win.document.open();
...
/**
 * Скачивает HTML-файл с инструкцией.
 * Пользователь может открыть его локально и распечатать через браузер.
 */
export function downloadInstructionsHTML(): void {
  ...
  a.href = url;
  a.download = `golden-investor-instructions-${new Date()
    .toISOString()
    .slice(0, 10)}.html`;
